      ...
      if(e.which == ARROW_DOWN || e.which == ARROW_UP){
        ...
        if(e.which == ARROW_DOWN){
            if($liSelected && $liSelected.next){
                ...
                // AFTER:
                // since pressing Enter, after typing text, opens the 1st tab result...
                // start the selection at the 2nd line (index 1)
                $liSelected = $allLi.eq(1).addClass('selected');
            }
        ...
        /* In fact, i don't see a need to use any of this code..it still works fine to me...
        ...
        if($liSelected && $liSelected.is(':visible') && $liSelected.hasClass('tab')){
          ...
          // AFTER: nothing! :)
          // hacking way to prevent the bug of opening the first tab
          // when using ARROW_DOWN key to select the listed tabs
